<script>
    var household_expense = [
        @foreach ($result['household_expense'] as $household_expense)
            {
            y: Object.values(@json($household_expense))[0],
            name: Object.keys(@json($household_expense))[0]
            },
        @endforeach
    ]

    $('#expense-chart').highcharts({
        chart: {
            animation: false,
            type: 'pie',
